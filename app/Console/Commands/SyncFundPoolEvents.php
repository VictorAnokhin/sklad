...
class SyncFundPoolEvents extends Command
{
    ...
    private function upsertEvent(string $network, string $packageId, string $eventType, string $moveEventType, array $event): void
    {
        ...
        $values = [
            ...
            'pool_shares' => in_array($eventType, ['withdraw', 'av8_unstake'], true) ? '0' : (string) ($parsed['pool_shares'] ?? ($eventType === 'av8_stake' ? $amountAv8 : '0')),
            'burned_pool_shares' => in_array($eventType, ['deposit', 'av8_stake'], true) ? '0' : (string) ($parsed['burned_pool_shares'] ?? ($eventType === 'av8_unstake' ? $amountAv8 : '0')),
