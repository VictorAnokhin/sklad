...
class TelegramAgent
{
    ...
    private function handleAiDialog(int|string $chatId, string $text, array $message): string
    {
        ...
        if ($this->shouldDelegateToBackend($text)) {
            ...
            // Извлекаем URL, если задача связана с сайтом
            $inputData = [
                'query' => $text,
                'chat_id' => $chatId,
                'language' => 'ru',
     ...
     * Ищет URL-подобные паттерны (domain.com, http://..., https://...).
     */
    private function extractUrlFromText(string $text): ?string
    {
        ...
        // Ищем домены вида example.com, example.com.ua, sub.example.org
    ...
    {
        $prompt = <<<'PROMPT'
Ты — AI-аналитик AV8 Capital. У тебя есть доступ к базе данных компании.

🧠 КОНФИГУРАЦИЯ БОТА:
Твои правила работы, инструкции и поведенческие настройки задаются администратором через панель /settings → База знаний → категория «telegram_instruction».
Эти инструкции переданы тебе ниже в разделе «🧠 ИНСТРУКЦИИ ТЕЛЕГРАМ-БОТА».
Следуй им как основной конфигурации — они имеют наивысший приоритет.

📚 БАЗА ЗНАНИЙ ПРОЕКТА:
Записи из базы знаний (раздел «📚 База знаний проекта») содержат проверенную информацию.
Используй их для ответов на вопросы пользователя.
