...
class FilterService
{
    ...
    public function resolve(string $doc, string $fid): array
    {
        ...
        if ($fName !== '') {
            $like = '%' . $fName . '%';
            $userSql .= ' AND (u.orgname LIKE ? OR u.name2 LIKE ? OR u.name LIKE ?
        ...
        if ($fContent !== '') {
            $like = '%' . $fContent . '%';
            $userSql .= ' AND (d.content LIKE ? OR d.ttn LIKE ? OR d.num LIKE ?)';
        ...
        if ($fdata1 !== '') {
            $ts = nextdate($fdata1);
            if ($ts) { $docSql .= ' AND d.dt >= ?'; $params[] = $ts; }
        }
        if ($fdata2 !== '') {
            $ts = nextdate($fdata2);
            if ($ts) { $docSql .= ' AND d.dt <= ?'; $params[] = $ts + 86399; }
        }
        if ($fMoneyWay !== '') {
        ...
        if ($fProvodka === '' && in_array($doc, ['WO1', 'ZD'], true)) {
        ...
        if ($doc === 'ZOUT') {
            if ($fStatus === '') {
                $hideIds = DB::table('conf')
                    ...
                    ->toArray();
                if (!empty($hideIds)) {
                    $ph      = implode(',', array_fill(0, count($hideIds), '?'));
                    $docSql .= " AND d.status NOT IN ({$ph})";
                    foreach ($hideIds as $hid) $params[] = $hid;
                }
            } elseif ($fStatus === '999') {
