        ...
        <form action="{{ route($filterRouteName) }}" method="get">
            <div class="money-filter-modal__grid">
                <div class="money-filter-modal__field">
                    <label>{{ $activeTab === 'orders' ? __('money.filter_search') : __('money.filter_transfer_search') }}</label>
                    <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" class="form-control" maxlength="30" data-money-filter-safe autocomplete="off">
                </div>
...
document.querySelectorAll('#moneyFilterModal [data-money-filter-safe]').forEach(function(input) {
